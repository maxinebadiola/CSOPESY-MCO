    ...
    while (!g_exit_flag) {
        ...
        if (current_process != nullptr) {
            ...
            while (current_process->instructions_executed < current_process->instructions_total && !g_exit_flag) {
                ...
                std::vector<std::string> singleInstruction = generateRandomInstructions(current_process->name, 1);
                try {
                    executeInstructionSet(singleInstruction);
                } catch (const std::exception& e) {
                    // Handle any exceptions during instruction execution
                    std::lock_guard<std::mutex> lock(outputMutex);
    ...
    while (!g_exit_flag) {
        ...
        if (current_process != nullptr) {
            ...
            if (current_process->remaining_quantum <= 0) {
                ...
                // Initialize process-specific variables when process starts (thread-local)
                variables.clear();
                variables["var1"] = 0;
                variables["var2"] = 0;
                variables["var3"] = 0;
            ...
            if (current_process->instructions_executed < current_process->instructions_total && !g_exit_flag) {
                ...
                // Generate and execute one instruction at a time for RR
                std::vector<std::string> singleInstruction = generateRandomInstructions(current_process->name, 1);
                try {
                    executeInstructionSet(singleInstruction);
                } catch (const std::exception& e) {
        ...
        } else { // No process assigned to this core
